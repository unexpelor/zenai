...
export async function GET(request) {
  ...
  const services = [];
  const notConfiguredDetail = locale === "en" ? "This service is not configured on the server." : "Layanan ini belum dikonfigurasi di server.";
  const pushNotConfigured = (name) => services.push({ name, status: "not_configured", detail: notConfiguredDetail, latencyMs: null });
  ...
  else pushNotConfigured("Supabase Database");
  ...
  } else pushNotConfigured("Tavily Market Search");
