...
export async function GET(request) {
  ...
  if (process.env.GROQ_API_KEY) services.push(await timedCheck("Groq", () => checkModels("https://api.groq.com/openai/v1/models", { Authorization: `Bearer ${process.env.GROQ_API_KEY}` }))); else services.push({ name: "Groq", status: "not_configured", detail: "Provider tidak dikonfigurasi pada environment ini." });
  if (process.env.OPENROUTER_API_KEY) services.push(await timedCheck("OpenRouter", () => checkModels("https://openrouter.ai/api/v1/models", { Authorization: `Bearer ${process.env.OPENROUTER_API_KEY}` }))); else services.push({ name: "OpenRouter", status: "not_configured", detail: "Provider tidak dikonfigurasi pada environment ini." });
  if (process.env.GEMINI_API_KEY) services.push(await timedCheck("Gemini", () => checkModels(`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(process.env.GEMINI_API_KEY)}`, {}))); else services.push({ name: "Gemini", status: "not_configured", detail: "Provider tidak dikonfigurasi pada environment ini." });
