    ...
    {[
      ...
      <button
        ...
        style={{
          ...
          background: tab === key ? (key === "health" ? "#EFF6FF" : "#EFF6FF") : "#ffffff",
          color: tab === key ? (key === "health" ? "#2563EB" : "#2563EB") : "#475569",
            ...
            >
              ...
              {tab === "health" && (
                <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
                  <div style={{ padding: "22px", borderRadius: "18px", border: `1px solid ${darkMode ? "#334155" : "#BFDBFE"}`, background: darkMode ? "#0f172a" : "#EFF6FF" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: "14px", alignItems: "center", flexWrap: "wrap" }}>
                      <div><div style={{ fontSize: "20px", fontWeight: "800", color: darkMode ? "#f8fafc" : "#064e3b" }}>Live System Verification</div><div style={{ marginTop: "6px", color: darkMode ? "#cbd5e1" : "#1D4ED8", fontSize: "14px", lineHeight: 1.5 }}>Pemeriksaan dilakukan saat tombol dijalankan dan hasil diberi timestamp.</div></div>
                      <button type="button" onClick={runLiveHealthCheck} disabled={healthLoading} style={{ border: "none", borderRadius: "10px", padding: "11px 16px", background: darkMode ? "#1D4ED8" : "#2563EB", color: "#fff", fontWeight: "800", cursor: healthLoading ? "wait" : "pointer" }}>{healthLoading ? "Memeriksa..." : "Run Live Health Check"}</button>
                    </div>
                  </div>
                  ...
                  {healthData && <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(2, minmax(0, 1fr))", gap: "14px" }}>
                      {(healthData.services || []).map((service) => <div key={service.name} style={{ padding: "18px", borderRadius: "14px", border: `1px solid ${darkMode ? "#334155" : "#e2e8f0"}`, background: darkMode ? "#111827" : "#fff" }}><div style={{ display: "flex", justifyContent: "space-between", gap: "12px" }}><strong>{service.name}</strong><span style={{ fontWeight: "800", color: service.status === "operational" ? "#2563EB" : service.status === "configured" ? "#b45309" : "#be123c" }}>{service.status === "operational" ? "● OPERATIONAL" : service.status === "configured" ? "● CONFIGURED" : "● DOWN"}</span></div><div style={{ marginTop: "8px", color: darkMode ? "#cbd5e1" : "#64748b", fontSize: "13px", lineHeight: 1.5 }}>{service.detail}</div>{service.latencyMs != null && <div style={{ marginTop: "8px", fontSize: "12px", color: darkMode ? "#94a3b8" : "#64748b" }}>Latency: {service.latencyMs} ms</div>}</div>)}
                    </div>
                    {healthData.liveAiSmokeTest && <div style={{ padding: "18px", borderRadius: "14px", border: `1px solid ${darkMode ? "#334155" : "#e2e8f0"}`, background: darkMode ? "#111827" : "#fff" }}><div style={{ display: "flex", justifyContent: "space-between" }}><strong>Live AI Smoke Test</strong><span style={{ color: darkMode ? "#60A5FA" : "#2563EB", fontWeight: "800" }}>● OPERATIONAL</span></div><div style={{ marginTop: "8px", fontSize: "13px", color: darkMode ? "#cbd5e1" : "#64748b" }}>Provider: {healthData.liveAiSmokeTest.provider} · Latency: {healthData.liveAiSmokeTest.latencyMs} ms · Response: {healthData.liveAiSmokeTest.response || "—"}</div></div>}
...
.zenai-dark [style*="#EFF6FF"],.zenai-dark [style*="#EFF6FF"] { background:#172554 !important; color:#D1FAE5 !important; border-color:#166534 !important; }
