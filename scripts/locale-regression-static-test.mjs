...
check("Latest locale wins", page.includes("aiRequestSeqRef") && page.includes("isLatestAiRequest") && page.includes("translationRequestRef.current.id"));
...
check("New AI states are registered", page.includes('registerCanonicalAi("growthActions"') && page.includes('registerCanonicalAi("decisionResult"') && page.includes("aiCanonicalRef"));
check("Persisted AI source locale metadata", page.includes("aiSourceLocales") && page.includes("saved.aiSourceLocales"));
check("DeepL API translation server", translateRoute.includes("DEEPL_API_KEY") && translateRoute.includes("/v2/translate") && translateRoute.includes("requestDeepL"));
